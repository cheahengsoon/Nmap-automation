                            ...
                            if tls_version:
                                ...
                                if cipher_table is not None:
                                    for cipher in cipher_table.findall("table"):
                                        ...
                                        cipher_list.append([f"{ip_address}[{ip_address_type}]", f"{port_id}[{port_protocol}]", cipher_name, kex_info, tls_version, security])
...
if uploaded_files:
    ...
    if xml_files:
        ...
        for uploaded_file in xml_files:
            ...
            with open(file_path+str(uuid.uuid4()), "wb") as f:
...
if st.session_state['files_uploaded']:
    all_ciphers = []
    uploaded_files = os.listdir(st.session_state['upload_folder'])
    for uploaded_file in uploaded_files:
        all_ciphers += parse_logic(uploaded_file=uploaded_file)    
    if all_ciphers:
...
if st.session_state['tls_cipher_eval']:
    ...
    insecure_df = cipher_df[~cipher_df['Security'].isin(['secure', 'recommended'])|~cipher_df['TLS Version'].isin(['TLSv1.2', 'TLSv1.3','Unknown'])]
    ...
    insecure_df['is_weak_encalg_sweet32'] = insecure_df.apply(lambda row: 'Y' if is_weak_encalg_sweet32(cipher_name=row['Cipher Name'],cipherdict=cipherdict) else 'N', axis=1)
    ...
    summary_df = insecure_df.groupby(['IP Address','Port']).agg({
